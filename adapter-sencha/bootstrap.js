...
(function () {
  ...
  */
  J = {
    ...
    * @type {Object}
    */
    adapterModules: {"sencha": ["uielement", "uielements/button", "uielements/list", "uielements/map", "uielements/panel", "uielements/spacer", "uielements/toolbar", "uielements/video.youtube"]},
  ...
  // 'joshfire/uielements/list' => 'joshfire/adapters/DEVICE/uielements/list'
  var addAdapterToDeps = function(deps) {
    ...
    for (var i = 0, l = deps.length; i < l; i++) {
      ...
      for (var ii = 0, ll = adaptersLookup.length; ii < ll; ii++) {
        //todo !joshfire
        src = deps[i].replace(/joshfire\//, '');

        // If the adapter provides an implementation of this class, load it instead
        if (J.adapterModules[adaptersLookup[ii]].indexOf(src) >= 0) {
          deps[i] = deps[i].replace(src, 'adapters/' + adaptersLookup[ii] + '/' + src);
          break;
        }
  ...
  */
  J.require = function(deps, callback) {
    ...
    if (J.debug && J.adapter != 'node') {
      ...
      }, addAdapterToDeps(deps), callback);
    } else {
      require(addAdapterToDeps(deps), callback);
    }
  };
